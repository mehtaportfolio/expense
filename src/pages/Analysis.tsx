...
export function Analysis({ mode }: AnalysisProps) {
  ...
  const handleExpenseTypeClick = (expenseType: string) => {
    ...
    <div className="p-2">
      ...
      <Tabs className="mb-8">
        <TabList className="flex space-x-1 p-1 rounded-lg mb-6">
          ...
          <Tab className="px-4 py-2 rounded-md text-sm text-black font-medium bg-yellow-100 dark:bg-yellow-100 hover:bg-yellow-800 hover:dark:bg-yellow-800 aria-[selected=true]:bg-blue-600 aria-[selected=true]:dark:bg-blue-600 data-[selected=true]">Yearly Expense</Tab>
        ...
        <TabPanel>
          ...
          <div className="space-y-6">
            {/* Expense Type Chart with Reset Button */}
            <div className="bg-card p-4 rounded-lg shadow border border-primary">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-primary">Expense Types (Click to select type)</h3>
                {selectedExpenseType && (
                  <button
              ...
              <BarChart
                data={selectedExpenseType ? getExpenseTypeDataFiltered(expenses, selectedExpenseType) : getExpenseTypeOverallData(expenses)}
                onBarClick={handleExpenseTypeClick}
              />
            </div>
            ...
            {selectedExpenseType && (
              <div className="bg-blue-350 dark:bg-blue-650 p-3 rounded-md border border-blue-200 dark:border-blue-800">
                <p className="text-sm font-medium text-primary">
                  Selected Expense Type: <span className="font-bold">{selectedExpenseType}</span>
                  {yearFilter && <span> • Year: <span className="font-bold">{yearFilter}</span></span>}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Year Wise Expenses Chart */}
              <div className="bg-card p-4 rounded-lg shadow border border-primary">
                <BarChart
                  data={selectedExpenseType ? getYearlyExpensesDataByType(expenses, selectedExpenseType) : getYearlyExpensesData(expenses)}
                  title={selectedExpenseType ? `Yearly ${selectedExpenseType} (Click to select year)` : "Yearly Expenses (Click to select year)"}
              ...
              <div className="bg-card p-4 rounded-lg shadow border border-primary">
                <BarChart
                  data={selectedExpenseType ? getMonthlyExpensesDataByType(expenses, selectedExpenseType, parseInt(yearFilter)) : getMonthlyExpensesDataAllYears(expenses)}
                  title={selectedExpenseType ? `Monthly ${selectedExpenseType} - ${yearFilter}` : "Monthly Expenses (All Years)"}
...
function getExpenseTypeOverallData(expenses: Expense[]) {
...
function getExpenseTypeDataFiltered(expenses: Expense[], selectedType: string) {
  const amount = expenses
    .filter(exp => exp.expense_type === selectedType)
...
function getYearlyExpensesDataByType(expenses: Expense[], expenseType: string) {
  const yearMap = new Map<number, number>();

  expenses
    .filter(exp => exp.expense_type === expenseType)
    .forEach(exp => {
...
function getMonthlyExpensesDataByType(expenses: Expense[], expenseType: string, year?: number) {
  ...
  expenses
    .filter(exp => exp.expense_type === expenseType && (year === undefined || new Date(exp.date).getFullYear() === year))
    .forEach(exp => {
...
function getMonthlyExpensesDataAllYears(expenses: Expense[]) {
  const monthlyData = Array.from({ length: 12 }, (_, i) => ({
    month: new Date(0, i).toLocaleString('default', { month: 'short' }),
    amount: 0
  }));
