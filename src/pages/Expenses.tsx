...
import { Bell, BellOff, PlusIcon, WalletIcon } from 'lucide-react';
...
export function Expenses({
  ...
  }, [expenses, filters, hasActiveFilters]);

  const totalAmount = useMemo(() => {
    return filteredExpenses.reduce((sum, exp) => {
      return exp.expense_type === 'income' ? sum + exp.amount : sum - exp.amount;
    }, 0);
  }, [filteredExpenses]);
  ...
  return <div className="space-y-6">
      ...
      {hasActiveFilters && (
        <div className="bg-card rounded-lg border border-primary p-4 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-secondary">Filtered Total</p>
              <p className={`text-2xl font-bold mt-1 ${totalAmount >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
            ...
            <div className={`w-12 h-12 rounded-full flex items-center justify-center ${totalAmount >= 0 ? 'bg-emerald-500/10' : 'bg-red-500/10'}`}>
              <WalletIcon className={`w-6 h-6 ${totalAmount >= 0 ? 'text-emerald-500' : 'text-red-500'}`} />
            </div>
          </div>
        </div>
      )}
